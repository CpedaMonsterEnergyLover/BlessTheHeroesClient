...
{
    public class RangedAttackVisualizer : MonoBehaviour
    {
        ...
        public void SetActive(bool isActive, Vector3 point = default)
        {
            if(point == default)
        ...
        public void SetArrowActive(bool isActive) => arrowSpriteTransform.gameObject.SetActive(isActive);

        public async UniTask Shoot(Transform point)
        {
            var arrow = EffectsManager.GetEffect<EffectArrow>();
            arrow.SetPosition(ArrowPosition);
            arrow.SetRotation(GetRotation(point.position));
            await arrow.Shoot(point);
        }
